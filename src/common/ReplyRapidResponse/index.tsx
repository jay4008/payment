...
class ReplyRapidResponse extends React.Component {
  ...
  componentDidUpdate(prevProps,prevState) {
    if((prevState.rrBody != this.state.rrBody) || (prevState.files != this.state.files)){
  ...
  submitRapidResponse = async ()=> {
    ...
    const filteredData = {
      body: rrBody,
      attachment: this.getFilteredAttachments(files.filter(x=>['completed'].includes(x.status)))
    }

    try {
      
      let res = await request('post','/rapid-response/reply/'+this.props.rapidResponseId,filteredData);
      this.setState({loader:false});
      showFlashMessage('success','Your reply posted successfully','success');
      this.props.onReplyRapidResponse(res.data);
      this.cancelModal();
    }
    catch(e) {
      console.log('error',e);
  ...
  render() {
    ...
    let { rrBody,files,disableBtn,loader } = this.state;
      ...
      <Modal
        ...
        <View style={[styles.centeredView, { backgroundColor: modalBackground }]}>
          ...
          <View style={[styles.modalView]}>
            <Ainput value={rrBody} onChangeText={(e:string)=>{this.setState({rrBody: e})}} style={styles.textArea} numberOfLines={10} placeholder='Enter your reply here'></Ainput>
          ...
          <Rbutton loader={loader} disabled={disableBtn} paddingV={15} style={styles.createBtn} onPress={()=>{this.submitRapidResponse()}}>Reply Now</Rbutton>
        </View>
